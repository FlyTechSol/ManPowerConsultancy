...
namespace MC.API.Controllers.Registration
{
    ...
    public class UserAssetController : ControllerBase
    {
        ...
        public async Task<ActionResult<ApiResponse<PaginatedResponse<UserAssetDetailDto>>>> GetAllByUserProfile(Guid userProfileId, [FromQuery] QueryParams queryParams, CancellationToken cancellationToken)
        {
            var response = await _mediator.Send(new GetAllByUserProfileQuery(userProfileId, queryParams), cancellationToken);
            return Ok(response);
        }
