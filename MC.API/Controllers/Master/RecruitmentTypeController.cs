...
namespace MC.API.Controllers.Master
{
    ...
    public class RecruitmentTypeController : ControllerBase
    {
        ...
        [HttpGet]
        public async Task<ActionResult<ApiResponse<PaginatedResponse<RecruitmentTypeDetailDto>>>> GetAll([FromQuery] QueryParams queryParams, CancellationToken cancellationToken)
        {
            var response = await _mediator.Send(new GetAllRecruitmentTypeQuery(queryParams), cancellationToken);
            return Ok(response);
        }

        [HttpGet("{id}")]
        public async Task<ActionResult<RecruitmentTypeDetailDto>> Get(Guid id, CancellationToken cancellationToken)
        {
            var response = await _mediator.Send(new GetByIdRecruitmentTypeQuery(id), cancellationToken);
            return Ok(response);
        }
        ...
        public async Task<ActionResult> Post(CreateRecruitmentTypeCmd request, CancellationToken cancellationToken)
        {
        ...
        public async Task<ActionResult> Put(UpdateRecruitmentTypeCmd request, CancellationToken cancellationToken)
        {
