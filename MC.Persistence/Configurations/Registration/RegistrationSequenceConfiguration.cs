...
namespace MC.Persistence.Configurations.Registration
{
    public class RegistrationSequenceConfiguration : IEntityTypeConfiguration<RegistrationSequence>
    {
        ...
        {
            //builder.Property(z => z.Prefix).IsRequired().HasMaxLength(10);
            //// 1. Set CompanyId as the Primary Key (assuming 1:1 with Company)
            //builder.HasKey(rs => rs.CompanyId);

            //// 2. Configure the required relationship to Company
            //builder.HasOne(rs => rs.Company)
            // .WithOne() // ✔️ Because it's 1:1
            // .HasForeignKey<RegistrationSequence>(rs => rs.CompanyId)
            // .IsRequired()
            // .OnDelete(DeleteBehavior.Restrict);

            //// 3. Configure the LastRegistrationId column
            //builder.Property(rs => rs.LastRegistrationId)
            //       .IsRequired();

            //// Optional: Unique constraint (redundant if PK = CompanyId)
            //builder.HasIndex(rs => rs.CompanyId).IsUnique();
