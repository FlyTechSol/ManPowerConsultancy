...
{
    public class UpdateAssetCmdValidator : AbstractValidator<UpdateAssetCmd>
    {
        private readonly IAssetRepository _assetRepository;

        public UpdateAssetCmdValidator(IAssetRepository assetRepository)
        {
        ...
        {
            var isUnique = await _assetRepository.IsUniqueForUpdate(id, code, cancellationToken);
            return isUnique;
        }

        private async Task<bool> RecordMustExist(Guid id, CancellationToken cancellationToken)
        {
            var record = await _assetRepository.GetByIdAsync(id, cancellationToken);
            return record != null;
        }
    }
}
